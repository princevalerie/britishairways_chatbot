...
# Konfigurasi model Gemini dengan streaming (gunakan model "gemini-2.0-flash")
generation_config = {
    "temperature": 0.2,
    "top_p": 0.95,
    "top_k": 40,
...
#######################################
# Fungsi Scraping, Chunking & Retrieval#
#######################################
def scrape_text(url):
    try:
        response = requests.get(url)
...
# Menampilkan total chunk yang dihasilkan (misalnya 5475)
st.sidebar.write(f"Total chunk yang dihasilkan: {len(all_chunks)}")

#######################
# UI Chatbot & Streaming#
#######################
st.title("Customer Review Analysis Chatbot with RAG & Streaming")
st.write("Tanyakan apapun tentang British Airways berdasarkan analisis review pelanggan.")
...
if user_input:
    ...
    with st.chat_message("assistant"):
        with st.spinner("Mencari informasi dan menghasilkan jawaban..."):
            # Cari chunk yang relevan menggunakan TF-IDF dan cosine similarity
            relevant_chunks = retrieve_relevant_chunks(user_input, all_chunks, top_n=3)
            context = "\n\n".join(relevant_chunks)
            
            # Buat pesan dengan instruksi sistem dan konteks data review
            instructions = "Gunakan data berikut untuk menganalisis dan menjawab pertanyaan mengenai review pelanggan:\n\n"
            message_text = f"{instructions}{context}\n\nPertanyaan: {user_input}"
            
            # Mulai sesi chat dengan model Gemini (menggunakan streaming)
            chat_session = model.start_chat(history=[{"role": "user", "parts": [{"text": message_text}]}])
            
            # Membuat placeholder untuk output streaming
            ...
            # Tambahkan jawaban ke history pesan
